...
void handle_command_mode(WINDOW *help_win, WINDOW *output_win, int socket_fd, char **file_list, int *file_count, int highlight)
{
    ...
    case 'r':
    {
        mvwprintw(help_win, 7, 1, "Enter new name for %s: ", file_list[highlight]);
        echo();
        curs_set(1);
    ...
    case 'c':
    {
        mvwprintw(help_win, 7, 1, "Enter permissions for %s (e.g., 777): ", file_list[highlight]);
        echo();
        curs_set(1);
        mvwgetnstr(help_win, 8, 1, input, sizeof(input) - 1);
        noecho();
        curs_set(0);
        char command[MAX_BUFFER_SIZE];
        snprintf(command, sizeof(command), "chmod %s %s", input, file_list[highlight]);
        execute_non_ls_command(socket_fd, command);
        refresh_file_list(socket_fd, output_win, file_list, file_count);
        break;
    }
...
int main()
{
    ...
    execute_ls_command(socket_fd, "ls", output_win, file_list, &file_count);
